...
import Genies from 0x631e88ae7f1d7c20
...
transaction(saleItemID: UInt64, saleItemPrice: UFix64, royaltyPercent: UFix64) {
    ...
    let nftProvider: Capability<&Genies.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(dapp: AuthAccount, seller: AuthAccount) {
        ...
        let nftProviderPrivatePath = /private/GeniesCollectionProviderForNFTStorefront
        let hasLinkedCollection = seller.
            getCapability<&Genies.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(
                nftProviderPrivatePath
            )!.check()
        if !hasLinkedCollection {
            seller.link<&Genies.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(
                nftProviderPrivatePath,
        ...
        if existingOffers.length > 0 {
            for listingResourceID in existingOffers {
                ...
                if listing != nil && listing!.getDetails().nftID == saleItemID && listing!.getDetails().nftType == Type<@Genies.NFT>(){
    ...
    pre {
        self.dappAddress == 0x631e88ae7f1d7c20: "Requires valid authorizing signature"
    }
